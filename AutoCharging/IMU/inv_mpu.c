...
#include "systick.h"
//#include "usart.h"
#define i2c_write   I2C_Write  //替换四个函数
...
int mpu_init(void )
{
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (mpu_set_accel_fsr(4))
        return -1;
    if (mpu_set_lpf(INV_FILTER_188HZ))
        return -1;
...
//        reg_int_cb(int_param);
    ...
    mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL);
		mpu_set_bypass(1); //setup bypass mode ,master directly control the aux i2c
    return 0;
}
...
//		data1 = (short)(tmp[1] << 8) | tmp[0];
//		data2 = (short)(tmp[3] << 8) | tmp[2];
//		data3 = (short)(tmp[5] << 8) | tmp[4];
//		USART_Printf(USART2,"%5d  %5d  %5d",data1,data2,data3);
...
//		USART_Printf(USART2,"COM:%5d %5d %5d\n",data[0],data[1],data[2]);

    data[0] = ((long)data[0] * st.chip_cfg.mag_sens_adj[0]) >> 8;
    data[1] = ((long)data[1] * st.chip_cfg.mag_sens_adj[1]) >> 8;
    data[2] = ((long)data[2] * st.chip_cfg.mag_sens_adj[2]) >> 8;

	//	USART_Printf(USART2,"COM1:%5d %5d %5d\n",data[0],data[1],data[2]);
    if (timestamp)
        get_ms(timestamp);
    return 0;
#else
